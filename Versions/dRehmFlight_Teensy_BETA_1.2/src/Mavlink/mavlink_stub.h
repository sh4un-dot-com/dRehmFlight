// Minimal MAVLink stub to allow "native" MAVLink compile-time branch to build
// This is NOT a full MAVLink implementation — it only provides the small API surface
// used by dRehmFlight (heartbeat + attitude packing + send buffer). The implementation
// serializes simple ASCII payloads so builds succeed and downstream tools can parse
// the messages if they expect the textual form.
